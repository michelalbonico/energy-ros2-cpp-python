...
for run_id in df['__run_id']:
    ...
    server_energy_files = glob.glob(os.path.join(folder_path, 'energy-server-*.csv'))
    if server_energy_files:
        try:
            server_energy_df = pd.read_csv(server_energy_files[0])
            avg_server_energy_pct = server_energy_df['CPU Utilization'].mean() * 100 * 8
...
plt.ylabel("CPU Usage (% of 1 core)")
plt.legend(title="Language")
plt.xticks(rotation=45)
plt.tight_layout()

# Save the plot
output_path = os.path.join(d_folder, 'server_cpu_barplot.png')
