...
def load_avg_memory_new(component: str, s_folder: str) -> pd.DataFrame:
    df = load_run_table()
    runs_data = {}

    for run_id in df['__run_id']:
        folder_path = os.path.join(s_folder, str(run_id))
        energy_file = os.path.join(folder_path, f'cpu-mem-{component}.csv')
        if os.path.exists(energy_file):
            try:
                energy_df = pd.read_csv(energy_file)
                avg_mem = energy_df['memory_usage'].mean() / 1024  # Convert to MB
                runs_data[run_id] = avg_mem
            except Exception as e:
                print(f"Error processing file for run_id {run_id}: {e}")

    avg_mem_df = pd.DataFrame(list(runs_data.items()), columns=['__run_id', 'avg_mem'])
    df = df.merge(avg_mem_df, on='__run_id', how='left')
...
def print_stats_new(grouped_data):
    for group_key, group in grouped_data:
        # Print all group keys
        group_key_str = ", ".join(f"{col}: {val}" for col, val in zip(grouped_data.keys, group_key))
        print(f"Group - {group_key_str}")
        print(group)
        print("-" * 50)
...
#grouped_data = load_total_energy('client',0.05)
grouped_data = load_avg_memory_new('server')
#grouped_data = load_avg_memory('server')
# print(grouped_data)
# grouped_data = grouped_data[grouped_data["msg_interval"] == 0.05]
print_stats_new(grouped_data)