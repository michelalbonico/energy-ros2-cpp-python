...
do
    ...
    python3 ../energy-mon.py -p simple_pub_sub_cpp -a talker -i pub_cpp_$_listeners &> ./log/talker-cpp.log &
    TALKER_PID=$!
    sleep 0.5

    for ii in $(seq 1 ${_listeners})
    do
        ros2 run simple_pub_sub_cpp listener &
        LISTENER_PID=$!
        sleep 0.1
    done
    
    sleep 1
    T_PID=`ps ax | grep simple_pub_sub_cpp/talker | head -1 | awk '{ print $1 }'`
    ...
    echo "$COUNT,pub_cpp_$_listeners,$TIME,$A_CPU,$A_MEM" >> ../data/resource-pubsub.csv
    let COUNT++
done