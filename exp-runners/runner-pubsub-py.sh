...
echo "exec,label,timestamp,cpu,mem" > ../data/resource-pubsub-py.csv
...
do
    ...
    python3 ../energy-mon.py -p simple_pub_sub_py -a talker -i pub_py4 &> ./log/talker-py.log &
    TALKER_PID=$!
    
    python3 ../energy-mon.py -p simple_pub_sub_py -a listener -i sub_py4 &> ./log/listener-py.log & 
    LISTENER_PID=$!
    
    sleep 1
    T_PID=`ps ax | grep ./runner-pubsub-py.sh | head -1 | awk '{ print $1 }'`
    L_PID=$T_PID
    ...
    echo "$COUNT,pub_py,$TIME,$A_CPU,$A_MEM" >> ../data/resource-pubsub-py.csv
    echo "$COUNT,sub_py,$TIME,$A_CPU_L,$A_MEM_L" >> ../data/resource-pubsub-py.csv
    let COUNT++
done